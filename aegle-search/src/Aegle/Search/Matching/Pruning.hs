...
renameP pren t =
  forceP t >>= \case
    VNe (HFlex m') sp -> case pren.occ of
      Just m | m == m' -> empty -- occurs check
      _ -> pruneVFlex pren m' sp
    VNe (HRigid (Level x)) sp -> case IM.lookup x pren.ren of
      Nothing -> empty -- scope error ("escaping variable" error)
      Just x' -> renameSpine pren (Var $ levelToIndex pren.dom x') sp
    VNe (HOpaque x) sp -> renameSpine pren (Opaque x) sp
    VNe (HAmb x) sp -> renameSpine pren (Amb x) sp
    VU -> pure U
    VPi x a b ->
      Pi x
        <$> renameP pren a
