...
import Aegle.Database.Feature qualified as TS
...
constructDefinition Definition' {..} = TS.Definition {..}
  where
    -- FIXME: allFeature should also do normalisation instead not to forget.
    -- Features are not invariant under isomorphism!
    (sig, _) = TS.nfIso [] signature
    feature = TS.allFeature sig
